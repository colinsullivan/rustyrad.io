 ...
 */
function rustyradio_customize_register( $wp_customize ) {
	...
	$wp_customize->add_setting( 'color_scheme', array(
		'default'           => 'default',
		'sanitize_callback' => 'rustyradio_sanitize_color_scheme',
		'transport'         => 'postMessage',
	) );

	$wp_customize->add_control( 'color_scheme', array(
		'label'    => __( 'Base Color Scheme', 'rustyradio' ),
		'section'  => 'colors',
		'type'     => 'select',
		'choices'  => rustyradio_get_color_scheme_choices(),
		'priority' => 1,
	) );
	...
	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'sidebar_textcolor', array(
		'label'       => __( 'Header and Sidebar Text Color', 'rustyradio' ),
		'description' => __( 'Applied to the header on small screens and the sidebar on wide screens.', 'rustyradio' ),
		'section'     => 'colors',
	) ) );
	...
	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'header_background_color', array(
		'label'       => __( 'Header and Sidebar Background Color', 'rustyradio' ),
		'description' => __( 'Applied to the header on small screens and the sidebar on wide screens.', 'rustyradio' ),
		'section'     => 'colors',
	) ) );

	// Add an additional description to the header image section.
	$wp_customize->get_section( 'header_image' )->description = __( 'Applied to the header on small screens and the sidebar on wide screens.', 'rustyradio' );
}
add_action( 'customize_register', 'rustyradio_customize_register', 11 );

/**
 * Register color schemes for Twenty Fifteen.
 *
 * Can be filtered with {@see 'rustyradio_color_schemes'}.
 *
 ...
 */
function rustyradio_get_color_schemes() {
	return apply_filters( 'rustyradio_color_schemes', array(
		'default' => array(
			'label'  => __( 'Default', 'rustyradio' ),
			'colors' => array(
				'#f1f1f1',
				'#ffffff',
				'#ffffff',
...
if ( ! function_exists( 'rustyradio_get_color_scheme' ) ) :
/**
 ...
 */
function rustyradio_get_color_scheme() {
	$color_scheme_option = get_theme_mod( 'color_scheme', 'default' );
	$color_schemes       = rustyradio_get_color_schemes();
...
endif; // rustyradio_get_color_scheme

if ( ! function_exists( 'rustyradio_get_color_scheme_choices' ) ) :
/**
 ...
 */
function rustyradio_get_color_scheme_choices() {
	$color_schemes                = rustyradio_get_color_schemes();
...
endif; // rustyradio_get_color_scheme_choices

if ( ! function_exists( 'rustyradio_sanitize_color_scheme' ) ) :
/**
 ...
 */
function rustyradio_sanitize_color_scheme( $value ) {
	$color_schemes = rustyradio_get_color_scheme_choices();
...
endif; // rustyradio_sanitize_color_scheme
 ...
 */
function rustyradio_color_scheme_css() {
	$color_scheme_option = get_theme_mod( 'color_scheme', 'default' );
	...
	$color_textcolor_rgb         = rustyradio_hex2rgb( $color_scheme[3] );
	$color_sidebar_textcolor_rgb = rustyradio_hex2rgb( $color_scheme[4] );
	$colors = array(
	...
	$color_scheme_css = rustyradio_get_color_scheme_css( $colors );

	wp_add_inline_style( 'rustyradio-style', $color_scheme_css );
}
add_action( 'wp_enqueue_scripts', 'rustyradio_color_scheme_css' );
...
function rustyradio_customize_control_js() {
	...
	wp_localize_script( 'color-scheme-control', 'colorScheme', rustyradio_get_color_schemes() );
}
add_action( 'customize_controls_enqueue_scripts', 'rustyradio_customize_control_js' );
...
add_action( 'customize_preview_init', 'rustyradio_customize_preview_js' );
 ...
 */
function rustyradio_get_color_scheme_css( $colors ) {
...
function rustyradio_color_scheme_css_template() {
	...
	?>
	<script type="text/html" id="tmpl-rustyradio-color-scheme">
		<?php echo rustyradio_get_color_scheme_css( $colors ); ?>
	</script>
	<?php
}
add_action( 'customize_controls_print_footer_scripts', 'rustyradio_color_scheme_css_template' );
